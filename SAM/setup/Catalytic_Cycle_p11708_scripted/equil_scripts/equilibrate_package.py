...
# set structure name via command line arg
parser = argparse.ArgumentParser(description='SETD8 Catalytic Cycle equilibration script')
parser.add_argument('structure_name', help='name of the structure to equilibrate')
...
print("System has %d particles" % system.getNumParticles())
...
print("Taking %d steps of dynamics" % minimization_nsteps)
simulation.step(minimization_nsteps)
print("Energy after dynamics is %s; minimizing" % (simulation.context.getState(getEnergy=True).getPotentialEnergy()))
