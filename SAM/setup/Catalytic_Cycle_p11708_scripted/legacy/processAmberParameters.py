# from https://github.com/juliebehr/AURKA_UMN/blob/TPO/fah-prep/phosphorylated/processAmberParameters.py
# using this one from Julie rather than the one in OpenMM as this one adds the name of the first residue to the atom type numbers
# this prevents clashes with the atom type numbers in the protein FF
# changes: changed last few lines to not allow both sigma, epsilon == 0,0 
# as is in here https://github.com/pandegroup/openmm/blob/master/devtools/forcefield-scripts/processAmberForceField.py
# only print out to the ffxml the residue called N6methyllysine.res - at least one other residue has a problem ForceField does not like - same atom names - so just include the one we need
# do not print out HarmonicBondForce, HarmonicAngleForce and PeriodicTorsionForce params - these will come from amber99sbildn.xml - don't want to duplicate them here in case problems. [cont. next line]
# Note we have to load in the .dat and frcmod files for ff99SBildn to provide L-J parameters here though hence making sure we don't print out and duplicate the unneeded params
import sys
import math
            ...
            if len(line) == 0 or first:
                ...
                for entry in line.split():
                    for field in entry.split('-'):
                        if field != '':
                            fields.append(field)
                bonds.append((fields[0], fields[1], fields[2], fields[3]))
                ...
                for entry in line.split():
                    for field in entry.split('-'):
                        if field != '':
                            fields.append(field)
                angles.append((fields[0], fields[1], fields[2], fields[3], fields[4]))
            elif block.startswith('DIHE'):
                fields = line[11:].split()
#                fields = list(filter(lambda a: a != '-', line.split()))
                periodicity = int(float(fields[3]))
                if continueTorsion:
                    torsions[-1] += [float(fields[1])/float(fields[0]), fields[2], abs(periodicity)]
                else:
...
name_mod = str(sorted(residueAtoms)[0])
def fix(atomClass):
    if atomClass == 'X':
        return ''
    return atomClass
...
for index, type in enumerate(types):
    ...
    print """  <Type name="%s" class="%s" element="%s" mass="%s"/>""" % (name_mod+'-'+str(index), type[0], el, mass)
print " </AtomTypes>"
print " <Residues>"
for res in sorted(residueAtoms):
    if res == 'N6methyllysine.res':
        print """  <Residue name="%s">""" % res
        for atom in residueAtoms[res]:
            print "   <Atom name=\"%s\" type=\"%s\"/>" % (tuple(atom)[0], name_mod+'-'+str(tuple(atom)[1]))
...
for index, type in enumerate(types):
    ...
    if sigma == 0 or epsilon == 0:
        sigma, epsilon = 1, 0    
    if q != 0 or epsilon != 0:
        print """  <Atom type="%s" charge="%s" sigma="%s" epsilon="%s"/>""" % (name_mod+'-'+str(index), q, sigma, epsilon)
